(() => {
  ...
  function setupPlayers(localName, remoteName) {
    state.players = {};
    state.playerOrder = [];
    const local = createPlayerState('local', localName, COLORS.blue);
    const remote = createPlayerState('remote', remoteName, COLORS.red);
    local.character = state.selectedCharacter;
    remote.character = 'mbappe';
    state.players[local.id] = local;
    state.players[remote.id] = remote;
    state.playerOrder.push(local, remote);
    state.localPlayerId = local.id;
    state.remotePlayerId = remote.id;
  }
  ...
  function triggerJuninhoCurve(player) {
    ...
    player.ability.juninhoCooldown = 30;
  }
  ...
  function handleKeyDown(event) {
    ...
    if (state.mode === 'playing') {
      const groundKey = event.code === controls.groundKick;
      const airKey = event.code === controls.airKick;
      const skillKey = event.code === controls.skill;
      if (groundKey) {
        event.preventDefault();
        if (network.role !== 'client') {
          startCharge(getLocalPlayer(), 'ground');
        }
      } else if (airKey) {
        if (network.role !== 'client') {
          startCharge(getLocalPlayer(), 'air');
        }
      } else if (skillKey) {
        event.preventDefault();
        if (network.role !== 'client') {
          tryActivateMbappeSkill(getLocalPlayer());
        }
      ...
      if (network.role === 'client') {
        sendNetworkMessage({ type: 'input', keys: buildNetworkInputKeys() });
      }
    }
  ...
  function handleKeyUp(event) {
    ...
    if (state.mode === 'playing') {
      ...
      if (groundKey) {
        if (network.role !== 'client') {
          releaseCharge(getLocalPlayer(), 'ground');
        }
      } else if (airKey) {
        if (network.role !== 'client') {
          releaseCharge(getLocalPlayer(), 'air');
        }
      }
      if (network.role === 'client') {
        sendNetworkMessage({ type: 'input', keys: buildNetworkInputKeys() });
  ...
  function drawMenu() {
    ...
    if (state.menuStep === 'mode') {
      ...
      ctx.fillText('Juninho: sut/pasta WASD yonlu falso (30sn bekleme)', state.view.w / 2, state.view.h * 0.68);
    } else {
